...
class Visual_MPC_Client():
    ...
    def run_trajectory(self, i_tr):
        ...
        if self.use_robot:
            ...
            if self.benchname != None:
                if self.args.save_subdir == "True":
                    self.save_subdir = raw_input('enter subdir to save data:')
                    self.desig_pix_img_dir = "/home/guser/catkin_ws/src/lsdc/experiments/cem_exp/benchmarks_sawyer/" + self.benchname + \
                                             '/' + self.save_subdir + "/videos"
                else:
                    self.desig_pix_img_dir = "/home/guser/catkin_ws/src/lsdc/experiments/cem_exp/benchmarks_sawyer/" + self.benchname + "/videos"
                if not os.path.exists(self.desig_pix_img_dir):
                    os.makedirs(self.desig_pix_img_dir)
            else:
                self.desig_pix_img_dir = self.recording_dir
